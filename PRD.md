...
| Database (local) | Docker Compose (Postgres) or PGlite | `npm run dev` works on day one, no cloud account |
| Database (prod) | Neon (default) or Turso | Indie-friendly pricing |
...
│       ├── .claude/
│       │   └── settings.json  # recommended hooks
│       ├── .env.example       # all keys with descriptions
│       └── package.json
├── apps/
│   └── web/                   # Next.js 15 reference app (mirrors templates/web)
├── AGENTS.md                  # root-level: monorepo purpose + contribution guide
├── CLAUDE.md                  # gstack section + skill routing rules
├── TODOS.md
├── CONTRIBUTING.md
├── scripts/
│   └── setup-gstack.sh        # gitignored; contributors run this after clone
└── .claude/
...
✓ happy path: Turso selected → drizzle.config uses @libsql/client
✓ happy path: tRPC excluded → no src/server/api/, no tRPC in package.json
...
18. Prompt flow (clack for prompts): name, database, tRPC, payments, email
...
28. Publish `create-pkstack@0.1.0` + `@pkstack/config@0.1.0`
